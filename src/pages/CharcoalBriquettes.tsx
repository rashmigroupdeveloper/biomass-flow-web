
import React from 'react';
import { motion } from 'framer-motion';
import Navigation from '@/components/Navigation';
import Footer from '@/components/Footer';
import { Button } from '@/components/ui/button';
import { ArrowRight } from 'lucide-react';
import { AspectRatio } from '@/components/ui/aspect-ratio';

const CharcoalBriquettes = () => {
  return (
    <div className="relative overflow-x-hidden bg-white">
      {/* Navigation */}
      <Navigation />
      
      {/* Hero Section */}
      <section className="relative pt-32 pb-20 md:pt-40 md:pb-28 px-6 md:px-12">
        <div className="max-w-6xl mx-auto">
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
            className="flex flex-col md:flex-row md:items-center"
          >
            <div className="md:w-1/2 mb-10 md:mb-0 md:pr-12">
              <h1 className="text-4xl md:text-6xl font-serif font-bold text-primary-800 mb-6">
                Charcoal Briquettes
              </h1>
              <p className="text-lg md:text-xl text-gray-600 mb-8">
                High-quality charcoal briquettes made from sustainable sources. Perfect for grilling, barbecuing, and commercial cooking applications.
              </p>
              <Button size="lg" className="rounded-full px-8 bg-primary-500 hover:bg-primary-600">
                Request a Quote <ArrowRight className="ml-2" />
              </Button>
            </div>
            <div className="md:w-1/2">
              <div className="rounded-2xl overflow-hidden shadow-xl">
                <AspectRatio ratio={4/3}>
                  <img 
                    src="https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05" 
                    alt="Charcoal Briquettes" 
                    className="w-full h-full object-cover"
                  />
                </AspectRatio>
              </div>
            </div>
          </motion.div>
        </div>
      </section>
      
      {/* Features Section */}
      <section className="py-20 px-6 md:px-12 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-serif font-bold text-primary-800 mb-12 text-center">
            Product Features
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {[
              {
                title: "Long Burning Time",
                description: "Our briquettes burn longer than standard charcoal, providing consistent heat for hours."
              },
              {
                title: "Sustainable Source",
                description: "Made from agricultural waste and renewable biomass materials."
              },
              {
                title: "Low Smoke",
                description: "Engineered to produce minimal smoke for a cleaner cooking experience."
              }
            ].map((feature, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
                viewport={{ once: true }}
                className="bg-white p-8 rounded-xl shadow-md"
              >
                <h3 className="text-xl font-bold text-primary-700 mb-4">{feature.title}</h3>
                <p className="text-gray-600">{feature.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>
      
      {/* Specifications Section */}
      <section className="py-20 px-6 md:px-12">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-serif font-bold text-primary-800 mb-12 text-center">
            Product Specifications
          </h2>
          
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-200">
              {[
                { label: "Heat Output", value: "8,000 kcal/kg" },
                { label: "Ash Content", value: "<5%" },
                { label: "Fixed Carbon", value: ">80%" },
                { label: "Moisture Content", value: "<5%" },
                { label: "Burning Time", value: "3-4 hours" },
                { label: "Packaging", value: "5kg, 10kg, 25kg bags" }
              ].map((spec, index) => (
                <div key={index} className="p-6">
                  <div className="text-sm text-gray-500">{spec.label}</div>
                  <div className="text-lg font-medium text-primary-700 mt-1">{spec.value}</div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </section>
      
      {/* Applications Section */}
      <section className="py-20 px-6 md:px-12 bg-primary-50">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-serif font-bold text-primary-800 mb-12 text-center">
            Applications
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              {
                title: "Home Barbecue",
                description: "Perfect for backyard grilling and family gatherings."
              },
              {
                title: "Commercial Restaurants",
                description: "Ideal for restaurants that require consistent heat for cooking."
              },
              {
                title: "Industrial Applications",
                description: "Used in various industrial heating applications."
              }
            ].map((app, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
                viewport={{ once: true }}
                className="bg-white p-8 rounded-xl shadow-md"
              >
                <h3 className="text-xl font-bold text-primary-700 mb-4">{app.title}</h3>
                <p className="text-gray-600">{app.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>
      
      {/* CTA Section */}
      <section className="py-20 px-6 md:px-12 bg-primary-800 text-white">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl font-serif font-bold mb-6">
            Ready to Order Charcoal Briquettes?
          </h2>
          <p className="text-lg md:text-xl text-primary-100 mb-10">
            Contact us today to discuss your charcoal briquette requirements and get a customized quote.
          </p>
          <Button size="lg" variant="outline" className="rounded-full px-8 border-white text-white hover:bg-white hover:text-primary-800">
            Contact Our Team
          </Button>
        </div>
      </section>
      
      {/* Footer */}
      <Footer />
      
      {/* Grain Texture Overlay */}
      <div 
        className="fixed inset-0 z-50 pointer-events-none opacity-[0.03]"
        style={{
          backgroundImage: 'url("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAGQAAABkCAYAAABw4pVUAAAOh0lEQVR4nO1dbVczNw69JCEvBJInEEIgEIb//6/6fbu73e77tt0P1o2vZQ3QPgk9nMP9dM+ZzIztWJYlWZJlnRsAxVUl58z5iLgyKbBCL/0oF09+fq6KiJB/tps3zXVFWZ3bGl+9/XP8tlh8V359qbMh/rr8fpr+7uSouhrfzKslOvtQnNS5vDh5StpEg9RSnydnyGnI+dzст")'
        }}
      ></div>
      
      {/* Page Transition */}
      <motion.div
        className="page-transition fixed inset-0 bg-primary-500 z-[100] pointer-events-none"
        initial={{ scaleY: 1 }}
        animate={{ scaleY: 0 }}
        transition={{ duration: 0.8, ease: [0.77, 0, 0.18, 1] }}
        style={{ transformOrigin: 'top' }}
      />
    </div>
  );
};

export default CharcoalBriquettes;

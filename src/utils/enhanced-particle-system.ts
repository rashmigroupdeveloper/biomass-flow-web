...
export class EnhancedBiomassParticleSystem {
  ...
  private _drawParticles(): void {
    // Group particles by color for batch drawing (reduces context switching)
    const particlesByColor: Record<string, EnhancedParticle[]> = {};
    
    for (let i = 0; i < this.particles.length; i++) {
      const p = this.particles[i];
      
      // Skip trail rendering in low performance mode
      if (!this.isLowPerformanceMode && this.options.trailEffect && p.trail && p.trail.length > 0) {
    ...
    Object.entries(particlesByColor).forEach(([color, particles]) => {
      // Set fill style once for all particles of this color
      ...
      particles.forEach(p => {
        ...
        if (!this.isLowPerformanceMode && this.options.particleGlow) {
          // Add subtle glow
          const glowSize = p.size * 2;
          const gradient = this.ctx.createRadialGradient(
            0, 0, 0,  // Start position and radius (center of current transform)
            0, 0, Math.max(0.1, p.size)  // Ensure minimum radius of 0.1
          );
